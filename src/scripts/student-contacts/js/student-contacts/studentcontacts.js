...
require(["es6-promise"], function(promise) {
  ...
  require(["jquery", "escl", "fetch",  "jquery.inputmask", "datatables", "parsley", "inputmask.extensions"], function($, escl, fetch) {
    ...
    // If fetch is already defined in browser, the fetch variable sent through requireJS's DI will be undefined.
    // If so, reset the local fetch var to the global fetch var.
    if (!fetch) {
